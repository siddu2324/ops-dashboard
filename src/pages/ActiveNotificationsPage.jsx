...
export default function ActiveNotificationsPage({ go }) {
  ...
  const filteredNotifications = activeAlerts.filter((n) => {
    const matchesSearch =
      (n.alertname || n.name || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase()) ||
      (n.grafana_folder || n.folder || "")
        .toLowerCase()
        .includes(searchTerm.toLowerCase());
    const matchesContact =
      contactPoint === "all" || n.contactPoint === contactPoint;
    ...
    <div className="max-w-7xl mx-auto space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-[var(--color-muted)]">
          See grouped alerts with active notifications
        </p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <option value="alertname,grafana_folder">
      ...
      {filteredNotifications.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-[var(--color-border)] rounded-lg bg-[var(--color-panel)]">
          ...
          <h3 className="text-[var(--color-text)] font-medium text-lg">
            No results
          </h3>
                      ...
                      <div className="space-y-1 mt-1">
                        {Object.entries(notif.instanceLabels || {}).map(([key, value]) => (
                          <div key={key} className="text-xs flex items-start gap-2">
                            ...
                            <span className="text-[var(--color-text)] font-mono break-all">
                              {value}
                            </span>
                          </div>
                        ))}
