...
export default function ApplicationsPage() {
  ...
  const generateApplications = () => {
    ...
    return serverInventory.map((server, index) => {
      ...
      const template = appTemplates[index % appTemplates.length];
      const health = status === 'up' ? 'Healthy' : status === 'warning' ? 'Warning' : 'Critical';

      return {
        id: `app-${server.id}`,
        name: `${template} on ${server.hostname}`,
        template: template,
        ...
        cpu: cpu,
        memory: memory,
        uptime: metrics.uptime || `${Math.floor(Math.random() * 365)}d ${Math.floor(Math.random() * 23)}h`,
        components: Math.floor(Math.random() * 8) + 3,
        componentsWithProblems: status === 'down' ? Math.floor(Math.random() * 3) : 0,
        alerts: status === 'down' ? Math.floor(Math.random() * 5) : 0,
        activeAlerts: status === 'down' ? Math.floor(Math.random() * 3) : 0,
        lastUpdated: new Date().toISOString(),
        ip: server.ip,
        os: server.os,
        location: server.location || "EAST",
        environment: server.os.includes("Windows") ? "Production" : "Staging",
