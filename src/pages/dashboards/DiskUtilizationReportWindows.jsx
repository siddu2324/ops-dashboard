...
export default function DiskUtilizationReportWindows() {
  ...
  const diskDataLinux = linuxServers.map(server => {
    ...
    const metrics = statusData?.metrics || { disk: 0 };
    return {
  ...
  const generateProblemsForHost = (hostname, status, statusData) => {
    ...
    const problemMap = {
      ...
      warning: `${hostname} has high disk usage`,
    ...
    const detail = {
      ...
      rootCause: status === "down"
        ...
        : status === "warning"
        ? `${hostname} is experiencing high disk usage (${statusData?.metrics?.disk || 0}%). This may affect application performance.`
      ...
      recommendation: status === "down"
        ...
        : status === "warning"
        ? "Clean up unnecessary files, archive old logs, and consider increasing disk capacity."
