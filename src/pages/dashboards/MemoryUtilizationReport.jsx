...
export default function MemoryUtilizationReport() {
  ...
  // Build data from serverStatuses
  const memoryDataWindows = windowsServers.map(server => {
  ...
  const generateProblemsForHost = (hostname, status, statusData) => {
    // Map status to severity and problem text
    const severityMap = {
      down: "Critical",
      warning: "Warning",
      up: "Information"
    ...
    // Use a single problem template that matches the actual status
    const detail = {
      title: `${hostname} · ${problemText}`,
      source: "Infrastructure",
      host: hostname,
      service: "Windows Server 2022",
      ...
      rootCause: status === "down"
        ...
        : status === "warning"
        ? `${hostname} is experiencing high resource usage (${statusData?.metrics?.memory || 0}% memory). This may affect performance.`
        : `${hostname} is operating normally.`,
      metrics: [
        `Status: ${status}`,
        `Memory utilization: ${statusData?.metrics?.memory || 0}%`,
      ...
      recommendation: status === "down"
        ...
        : status === "warning"
        ? "Investigate resource-consuming processes. Consider scaling or redistributing workloads."
        : "No action required. System is healthy."
    };

    // Return a single problem (or multiple if you want, but we'll keep it simple)
    return [{
      id: 1,
  ...
  const handleRowClick = (hostname) => {
    ...
    // Only show problems if the host is not healthy
    if (status !== "up") {
      ...
      // For healthy hosts, show a notification (or optionally open a modal with "No problems")
