// src/pages/dashboards/FirewallRealTimeService.jsx
...
const ProblemDetailModal = ({ isOpen, onClose, row }) => {
  ...
  const isProblem = row.status === "Stopped";
  const title = isProblem ? `${row.host} · ${row.service} Service Stopped` : `${row.host} · ${row.service} Service Running`;
  const severity = isProblem ? "High" : "Information";
  const confidence = isProblem ? "88%" : "100%";
  const rootCause = isProblem
    ? `The ${row.service} service on ${row.host} is stopped. This may be due to a process crash, configuration error, or manual stop.`
    : `The ${row.service} service on ${row.host} is running normally.`;
  ...
  const recommendation = isProblem
    ? "Restart the service and check logs for errors. Verify service dependencies and configuration files. If the issue persists, investigate the root cause."
      ...
      <div className="bg-[var(--color-panel)] border border-[var(--color-border)] rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <span className="text-xs bg-[var(--color-panel-alt)] px-2 py-0.5 rounded border border-[var(--color-border)] text-[var(--color-muted)]">Service Status</span>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-140px)] space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-2 text-sm">
            ...
            <span className="text-xs font-mono text-[var(--color-faint)]">Reference ID SRV-{row.service}</span>
          </div>

          <div className="bg-[var(--color-bg)] rounded-xl border border-[var(--color-border)] p-4">
            <p className="text-sm text-[var(--color-text)]">Live service snapshot for {row.host} - {row.service}.</p>
          </div>

          <div>
            <div className="flex items-center gap-3 mb-2">
              <h4 className="text-sm font-semibold text-[var(--color-text)] uppercase tracking-wider">Root Cause Analysis</h4>
              <span className="text-xs font-bold text-[var(--color-accent)] bg-[var(--color-accent)]/10 px-2 py-0.5 rounded-full border border-[var(--color-accent)]/20">{confidence} CONFIDENCE</span>
            </div>
            ...
            <tbody>
              {data.map((row, i) => (
                <tr key={i} onClick={() => handleRowClick(row)} className="border-b border-[var(--color-border)] hover:bg-[var(--color-border)]/10 transition cursor-pointer group">
                  <td className="py-2 px-3 text-[var(--color-text)]">{row.service}</td>
                  ...
                  <td className="py-2 px-3 text-[var(--color-text)]">{row.uptime}</td>
