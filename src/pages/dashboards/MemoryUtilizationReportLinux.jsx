...
export default function MemoryUtilizationReportLinux() {
  ...
  const memoryDataLinux = linuxServers.map(server => {
    ...
    const metrics = statusData?.metrics || { memory: 0 };
    return {
      ip: server.ip,
  ...
  const generateProblemsForHost = (hostname, status, statusData) => {
    ...
    const problemMap = { down: `${hostname} is unreachable`, warning: `${hostname} has high memory usage`, up: `${hostname} is healthy` };
    ...
    const detail = {
      ...
      rootCause: status === "down"
        ? `${hostname} is under elevated network unreachability (100%). This has crossed the critical threshold (85%) and is likely impacting workloads.`
        : status === "warning"
        ? `${hostname} is experiencing high memory usage (${statusData?.metrics?.memory || 0}%). This may affect performance.`
      ...
      recommendation: status === "down"
        ? "Check network connectivity, firewall rules, and ensure the host is powered on."
        : status === "warning"
        ? "Investigate memory-consuming processes. Consider scaling or redistributing workloads."
  ...
  return (
    <div className="space-y-4">
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {memoryDataLinux.map((row) => (
                <tr key={row.ip} onClick={() => handleRowClick(row.hostname)} className="border-b border-[var(--color-border)] hover:bg-[var(--color-panel-alt)] transition-colors cursor-pointer group">
      ...
      <HostProblemsModal isOpen={modalOpen} onClose={closeModal} hostname={selectedHost} problems={problems} />
    </div>
  );
}